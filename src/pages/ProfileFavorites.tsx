...
export default function ProfileFavorites() {
  ...
  useEffect(() => {
    ;(async () => {
      ...
      if (!favorites || favorites.length === 0) { setLoading(false); return }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center mb-8">
        <h2 className="text-2xl tracking-wider" style={{ fontFamily: "'Cinzel', serif", color: 'var(--color-ivory)' }}>FAVORITES</h2>
        <p className="text-sm mt-1" style={{ color: 'var(--color-muted)', fontFamily: "'Noto Serif SC', serif" }}>我的 · 收藏</p>
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          <p className="text-sm" style={{ color: 'var(--color-muted)', fontFamily: "'Noto Serif SC', serif" }}>暂无收藏</p>
          <p className="text-xs mt-2 opacity-60" style={{ color: 'var(--color-muted)' }}>收藏第一封信...</p>
        </div>
      ) : (
