...
function Profile() {
  const [profiles, setProfiles] = useState<ProfileData[]>(() => {
    try {
      const cached = sessionStorage.getItem('sa_profiles')
      if (cached) {
  ...
  const handleAvatarChange = async (id: string, file: File) => {
    ...
    if (url) {
      ...
      sessionStorage.setItem('sa_profiles', JSON.stringify(profiles))
    }
  }
  ...
  return (
    <div className="space-y-6">
      {/* 页面标题 */}
      <div className="text-center mb-8">
        <h2 className="text-2xl tracking-wider" style={{ fontFamily: "'Cinzel', serif", color: 'var(--color-ivory)' }}>PROFILE</h2>
        <p className="text-sm mt-1" style={{ color: 'var(--color-muted)', fontFamily: "'Noto Serif SC', serif" }}>我的 · 人机档案</p>
      ...
      <Card>
        <div className="flex items-center justify-center gap-6">
            ...
            {editingId === human?.id ? (
              <div className="mt-2 max-w-[150px] mx-auto">
                <div className="flex items-center gap-2 justify-center">
                  <input type="text" className="bg-[var(--color-coal)] border-[var(--color-brass)] text-[var(--color-paper)] px-2 py-1 rounded text-sm w-full max-w-[120px] text-center" value={tempName} onChange={(e) => setTempName(e.target.value)} onKeyDown={(e) => human && handleKeyDown(e, human.id)} autoFocus />
          ...
          <div className="flex flex-col items-center mx-3 text-center">
            ...
            <span className="text-xs mt-1" style={{ color: 'var(--color-brass)', fontFamily: "'EB Garamond', serif", fontStyle: 'italic', letterSpacing: '0.05em' }}>∞</span>
          </div>
          <div className="flex flex-col items-center">
            ...
            {editingId === ai?.id ? (
              <div className="mt-2 max-w-[150px] mx-auto">
                <div className="flex items-center gap-2 justify-center">
                  <input type="text" className="bg-[var(--color-coal)] border-[var(--color-brass)] text-[var(--color-paper)] px-2 py-1 rounded text-sm w-full max-w-[120px] text-center" value={tempName} onChange={(e) => setTempName(e.target.value)} onKeyDown={(e) => ai && handleKeyDown(e, ai.id)} autoFocus />
      ...
      <Card delay={200}>
        <Link to="/profile/favorites" className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div><p className="text-sm font-medium" style={{ color: 'var(--color-ink)', fontFamily: "'Noto Serif SC', serif" }}>收藏</p><p className="text-xs" style={{ color: 'var(--color-muted)' }}>收藏的帖子与内容</p></div>
          </div>
      ...
      <Card delay={300}>
        <Link to="/profile/comments" className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="text-lg">✎</span>
            <div><p className="text-sm font-medium" style={{ color: 'var(--color-ink)', fontFamily: "'Noto Serif SC', serif" }}>评论</p><p className="text-xs" style={{ color: 'var(--color-muted)' }}>发出与收到的评论</p></div>
          </div>
          <span className="text-sm" style={{ color: 'var(--color-brass)' }}>›</span>
        </Link>
      </Card>
      <Card delay={400}>
        <Link to="/profile/messages" className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="text-lg">✉</span>
            <div><p className="text-sm font-medium" style={{ color: 'var(--color-ink)', fontFamily: "'Noto Serif SC', serif" }}>私信</p><p className="text-xs" style={{ color: 'var(--color-muted)' }}>收发的私信记录</p></div>
          </div>
          <span className="text-sm" style={{ color: 'var(--color-brass)' }}>›</span>
        </Link>
      </Card>
      <Card delay={500}>
        <Link to="/profile/posts" className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="text-lg">❦</span>
            <div><p className="text-sm font-medium" style={{ color: 'var(--color-ink)', fontFamily: "'Noto Serif SC', serif" }}>帖子</p><p className="text-xs" style={{ color: 'var(--color-muted)' }}>发布过的所有帖子</p></div>
          </div>
          <span className="text-sm" style={{ color: 'var(--color-brass)' }}>›</span>
        </Link>
      </Card>
